...
- `"UserName"` が `PascalCase` であると判別できる。
- `"PLAYER_ACTION_WALKING"` を `"playerActionWalking"` に変換できる。
- `"m_UserName"` から、プレフィックスあり・プレフィックス除去後シンボルが `"UserName"` であることを取得できる。
- `"__m_user_name__"` から、解析用に正規化したシンボル `"user_name"` と、先頭/末尾アンダースコア数を取得できる。
- `"Player"` のような曖昧入力（`PascalCase` または `UpperSnakeCase`）を `Unknown` として認識しつつ、ポリシー指定でプロジェクト方針に合わせて判定挙動を制御できる。
