...
- Conversion API は、単純な `string` 戻り値だけでなく将来拡張しやすい設計（Result 型や Token ベース）を優先する。ユーザーは Conversion Result 型導入で、既存の string 戻り値メソッドを並存させる非破壊対応よりも、APIの使いやすさと長期保守性を優先して破壊的変更（戻り値を Result 型へ一本化）を支持している。
...
- 利用者向けREADMEは章単体ではなく全体を通読したときの理解順で構成し、未説明の用語・機能名を先に出さず、まず利用シナリオを示してからAPI名を出す。ライブラリ実装者向けの内部意図・内部動作説明（例: Freezeの内部制御理由）は載せず、公開APIの利用に必要な情報だけを書く。READMEはライブラリ開発者都合ではなく利用者が迷わない記述を優先し、前提説明なしの機能名（例: Freeze）を出さない。必要なら、どの利用形態で必要か・不要かをサンプル付きで明示する。
- READMEにはSpan対応だけでなく、アナライザの並列実行を想定した高性能（CPU/メモリ）・スレッドセーフ/ステートレス利用方針と専用オーバーロードの有無を明記する。実装もランタイム利用を想定して可能な範囲で高性能化を優先する。
- READMEのサンプルは実際のプロダクトコードにありえる実践的な値を使い、PrefixProviderとProtectedWordProviderの役割差が直感的に伝わるようにする。例として `str` はPrefix、`iOS` はProtectedWord。
...
## 高頻度実行と最適化
- ユーザーは本ライブラリをアナライザ等の超並列・高頻度実行用途として想定しており、早期段階から `Span`/`Memory` などの最適化技法を必須要件として全タスク（T3だけでなく全体）に組み込む設計を求めている。
