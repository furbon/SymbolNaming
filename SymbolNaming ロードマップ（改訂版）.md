...
- [x] `NormalizeForAnalysis`（仮名）を追加する
- [x] 正規化結果と装飾情報（先頭/末尾アンダースコア等）を結果モデルとして返す
- [x] 利用側が `string.StartsWith` 依存なしで判定できるようにする

### 性能要件（必須）

- [x] 可能な限り `Span`/スライス情報中心で返す
- [x] 余分な文字列生成を抑制する

### 受け入れ条件

- [x] `Analyze` / `Inspect` と情報整合がある
- [x] 特定ケース特化ではなく一般化設計になっている
- [x] README で用途分担（正規化 vs 解析）が明確である
...
- [x] ルール構築時の検証（不正パターン早期検出）を追加する
- [x] 高頻度評価向けに実行時マッチング経路を最適化する
- [x] 必要に応じてルール ID 重複などの整合チェックを追加する

### 性能要件（必須）

- [x] 実行時の正規表現関連オーバーヘッドを削減する
- [x] 反復呼び出し時の割り当てプロファイルを安定化する

### 受け入れ条件

- [x] 既存の `CompositePatternMatcher` 契約互換を維持する
- [x] ゲーム開発向け可変サフィックス要件を維持する
- [x] 性能上の意図と結果が文書化されている
...
- [x] 責務分離を維持する
- [x] 既存の確定挙動（特にトークナイズ）を壊さない
- [x] 変更ごとにテストと README を同時更新する
- [x] 共有インスタンスの並列利用前提を維持する
- [x] 仕様未確定領域（例: `m__UserName`）は独断で確定しない
...
- `M0`〜`M5` の主要 API 実装はコード上で確認済み。
- `M3` のベンチマーク結果はリポジトリ内に成果物がないため、受け入れ条件は未完了扱いに戻した。
- `M1` の移行情報は `README.md` に追記済み（`Convert` の戻り値変更と `result.Output` 利用手順）。
